...
class APIService
{
    ...
    public function requestJWTToken($user_id, $user_password)
    {
        ...
        $response = $this->requestApi('POST', $uri, '', $contentType, $body);

        return $this->parseResponseToken($response);
    }

    public function requestApi($method, $uri, $ipAddress, $contentType = '', $body = [], $requiredToken = false)
    {
        $actual_error = '';
        $status_code = 500;
        ...
        try {
            ...
            Log::info("[API-SERVICE] Request to " . env('API_SERVER') . $uri);

            // melakukan request ke API
            $response = $this->_guzzle_client->request($method, env('API_SERVER') . $uri, $options);
